...
import { LockingTab } from '../components/my-trades/LockingTab';
import { ReadyToSettleTab } from '../components/my-trades/ReadyToSettleTab';
import { HistoryTab } from '../components/my-trades/HistoryTab';
import { toast } from 'sonner@2.0.3';

export interface Trade {
  id: string;
  role: 'Maker' | 'Taker';
  status: 'locking' | 'ready_to_settle' | 'settled' | 'refunded' | 'expired_refundable';
...
export function MyTradesPage() {
  const [activeTab, setActiveTab] = useState<'locking' | 'settle' | 'history'>('locking');
  ...
  const [trades, setTrades] = useState<Trade[]>([
    {
      ...
      targetAmount: 10,
      targetToken: 'WETH',
      fillTime: new Date(Date.now() - 3 * 60 * 60 * 1000), // 3 hours ago
    ...
    },
    {
      ...
      targetAmount: 15000,
      targetToken: 'USDC',
      fillTime: new Date(Date.now() - 7 * 60 * 60 * 1000), // 7 hours ago
    ...
    },
    {
      ...
      targetAmount: 16,
      targetToken: 'WETH',
      fillTime: new Date(Date.now() - 13 * 60 * 60 * 1000), // 13 hours ago
    ...
    },
    {
      ...
      targetAmount: 8,
      targetToken: 'WETH',
      fillTime: new Date(Date.now() - 8 * 24 * 60 * 60 * 1000), // 8 days ago
    ...
    },
    {
      ...
      settledTime: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000 - 10 * 60 * 60 * 1000),
      finalVWAP: 3038,
    ...
    },
    {
      ...
      settledTime: new Date(Date.now() - 4 * 24 * 60 * 60 * 1000 - 11 * 60 * 60 * 1000),
      finalVWAP: 3048,
      receivedAmount: 13,
    ...
    },
    {
      ...
      settledTime: new Date(Date.now() - 6 * 24 * 60 * 60 * 1000 - 10 * 60 * 60 * 1000),
      finalVWAP: 3035,
  ...
  const readyToSettleTrades = trades.filter(t => 
    t.status === 'ready_to_settle' || t.status === 'expired_refundable'
  );
  const historyTrades = trades.filter(t => 
    t.status === 'settled' || t.status === 'refunded'
  );
  ...
  const handleSettle = (tradeId: string) => {
    setTrades(trades.map(trade => {
      if (trade.id === tradeId) {
        ...
        const received = trade.role === 'Taker' 
          ? trade.targetAmount 
          : trade.targetAmount * 1.02; // Mock 2% profit
        const refunded = trade.role === 'Taker'
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 className="text-3xl font-semibold text-gray-900 dark:text-white mb-8">My Trades</h1>
