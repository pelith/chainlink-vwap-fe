...
export function FillOrderModal({ order, onClose, onConfirm }: FillOrderModalProps) {
  const [depositAmount, setDepositAmount] = useState('');
  const [isApproved, setIsApproved] = useState(false);

  const isSellWeth = order.direction === 'SELL_WETH';
  const depositToken = isSellWeth ? 'USDC' : 'WETH';
  const receiveToken = isSellWeth ? 'WETH' : 'USDC';

  const depositAmountNum = parseFloat(depositAmount) || 0;
  const hasError = depositAmount !== '' && depositAmountNum < order.minAmountOut;
  ...
  // Calculate estimated settlement time (12 hours from now)
  const now = new Date();
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      {/* Backdrop */}
      <div className="fixed inset-0 bg-gray-900 bg-opacity-60 dark:bg-opacity-80 transition-opacity" onClick={onClose}></div>
      ...
      <div className="flex min-h-full items-center justify-center p-4">
        <div className="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-lg w-full p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-semibold text-gray-900 dark:text-white">Fill Order #{order.id}</h2>
            <button
              onClick={onClose}
          ...
          <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-gray-600 dark:text-gray-400">You are filling</span>
              <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium ${
                isSellWeth ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400' : 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400'
              }`}>
          ...
          {hasError && (
            <div className="mb-4 flex items-start space-x-2 text-red-600 dark:text-red-400 text-sm">
              ...
              <span>Amount must be at least {formatMinAmount(order.minAmountOut, depositToken)} {depositToken}</span>
            </div>
          )}
          ...
          {!hasError && depositAmount === '' && (
            <p className="mb-4 text-sm text-gray-500 dark:text-gray-400">
              Minimum deposit: {formatMinAmount(order.minAmountOut, depositToken)} {depositToken}
            </p>
          )}
          ...
          <div className="mb-6 border border-gray-200 dark:border-gray-700 rounded-lg p-4 space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-medium text-gray-900 dark:text-white">{settlementFormatted}</span>
            </div>
          </div>
            ...
            <div className="flex items-start space-x-2">
              ...
              <div className="text-sm text-amber-900 dark:text-amber-200">
                ...
                <p>
                  ...
                  Excess funds will be automatically refunded based on the final VWAP settlement price.
                </p>
              </div>
            </div>
          </div>
