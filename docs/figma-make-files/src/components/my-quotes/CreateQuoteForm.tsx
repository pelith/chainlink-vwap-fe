...
export function CreateQuoteForm({ onSubmit }: CreateQuoteFormProps) {
  const [direction, setDirection] = useState<'SELL_WETH' | 'SELL_USDC'>('SELL_WETH');
  ...
  const handleAutoCalculate = () => {
    ...
    const marketPrice = 3050; // Mock market price
    
    if (direction === 'SELL_WETH') {
      // Selling WETH, need min USDC
      const minUsdc = amountNum * marketPrice * 0.8; // 80% of market value
      setMinAmountOut(minUsdc.toFixed(0));
    } else {
      // Selling USDC, need min WETH
      const minWeth = (amountNum / marketPrice) * 0.8;
      setMinAmountOut(minWeth.toFixed(2));
    }
  };
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-6 sticky top-8">
      <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-6">Create New RFQ Order</h2>
        ...
        <div>
          ...
          <div className="mt-2 p-3 bg-blue-50 dark:bg-blue-900/30 rounded-lg">
            <p className="text-sm text-blue-900 dark:text-blue-200">
              Final Price = 12H VWAP × (1 {isPositiveDelta ? '+' : ''} {deltaPercent}%)
            </p>
